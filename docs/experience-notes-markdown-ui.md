...
4. **代码块分层**（2026-08）：`react-markdown` 的 fenced `code` 必须看 `inline`——`inline={false}` 才是围栏块，不能套用行内灰底样式（否则会出现「黑底 + 每行灰块」）。展示规则：`prompt` → 浅灰 Prompt 卡；`bash/sh/...` → 深色终端；无语言 / `text` / `env` / `json` 等配置类 → 浅色可复制块（适合 URL 列表、环境变量）。
